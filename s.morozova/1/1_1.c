...
void change_ulimit(const char *value){
    ...
    if (new_ulimit > rlim.rlim_max) {
        rlim.rlim_max = (rlim_t)new_ulimit;
    }
...
void print_usage(const char *program_name) {
    ...
    printf("  -p  Print process IDs (PID, PPID, PGID)\n");
    printf("  -u  Print ulimit value\n");
    printf("  -U <value>  Change ulimit value\n");
...
int main(int argc, char *argv[]) {
    ...
    while ((param = getopt(argc, argv, "ispuU:cC:dvV:h")) != -1){
        switch (param){
            case 'i':
                print_ids();
                break;
