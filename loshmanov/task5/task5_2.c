...
// Строим таблицу по отображённому файлу. Учитываем CRLF.
...
// Новая функция: печать таблицы отступов и длин
static void print_table(const Array *table, size_t file_size)
{
    printf("Lines table (file size: %zu bytes, lines: %d)\n", file_size, table->cnt);
    printf("%6s  %14s  %14s\n", "Line#", "Offset (bytes)", "Length (bytes)");
    printf("-----------------------------------------------------\n");
    for (int i = 0; i < table->cnt; ++i)
    {
        const Line L = table->array[i];
        // off_t печатаем через intmax_t для переносимости
        printf("%6d  %14" PRIdMAX "  %14" PRIdMAX "\n",
               i + 1, (intmax_t)L.offset, (intmax_t)L.length);
    }
    printf("\n");
}
...
int main(int argc, char *argv[])
{
    ...
    // === Печать таблицы отступов и длин ===
