/*
 * protedit.c — «защищённый редактор» для Solaris SVR4.
 * Идея: захватываем весь файл эксклюзивно через fcntl(F_SETLKW),
 * затем запускаем редактор через system(3), после выхода — снимаем захват.
 * Для mandatory-режима пользователь сам включает атрибут файла: `chmod +l FILE`
 * (см. лекцию по захватам: mandatory включается атрибутом файла; не работает по NFS).
 */
...
/* Простая безопасная оболочка для имени файла в одинарных кавычках
 * для передачи в shell-команду. Меняем ' на '\''.
 * Возвращает malloc-нутую строку вида:  'имя c экранированными кавычками'
 */
